...
predictor = TabularPredictor.load("E:/wheatN/Yield", require_version_match=False)
...
print("=== Surrogate → Ensemble Fitting metrics (training/testing)===")
...
plt.tight_layout()
plt.savefig(os.path.join(OUT_DIR, "shap-yield-dot.tif"), dpi=300)
plt.show()
...
importance.to_csv(os.path.join(OUT_DIR, 'yield_ensemble_surrogate_shap_importance_onehot.csv'), index=False, encoding="utf-8-sig")
